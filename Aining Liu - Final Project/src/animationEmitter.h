...
public:
	...
	void update()  {
		if (radius >= MAX_RADIUS) running = false;
		if (running){
...
public:
	...
	void update() {
		for (size_t i = 0; i < explosion_animations.size(); i++){
			...
			if (explosion_animations[i].running == false) explosion_animations.erase(explosion_animations.begin() + i);
		}
		for (size_t i = 0; i < waterhole_animations.size(); i++){
			waterhole_animations[i].update();
			if (waterhole_animations[i].running == false) waterhole_animations.erase(waterhole_animations.begin() + i);
		}
	}

	void draw() {
		for (size_t i = 0; i < explosion_animations.size(); i++) explosion_animations[i].draw();
		for (size_t i = 0; i < waterhole_animations.size(); i++) waterhole_animations[i].draw();
	}
};
